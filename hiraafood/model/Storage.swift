...
class Storage {
    ...
    static func loadCart() -> Cart {
        do {
            ...
            NSLog("loading cart... from \(fileURL)")
            let text = try String(contentsOf: fileURL, encoding: .utf8)
            let json:Data? = text.data(using: .utf8)
            return
                try JSONDecoder().decode(Cart.self, from:json!)
        } catch {
            NSLog("error while loading cart from file. Would be returning an empty cart")
            NSLog(String(describing: error))
            return Cart()
        }
    }
    ...
    static func ensureFileExits(filepath:URL) {
        ...
        if (!filemgr.fileExists(atPath: filepath)) {
            NSLog("Creating file with empty JSON data \(filepath)")
